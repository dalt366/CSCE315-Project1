...
{		
	...
	/*Test of cross join function for two tables*/
		...
		TEST_METHOD(crossJoin_SameSizeA){ 
			...
			t1.insert(r4);
			Table t3 = t.crossJoin(t, t1);
			Assert::AreEqual(t3.getSize(), 4); //(tamu, john), (tamu, jill), (tu, john), (tu, jill)
		}
		...
		TEST_METHOD(crossJoin_FullEmptyA1){ 
			...
			Table t1(m1,s1);
			Table t3 = t.crossJoin(t, t1);
			Assert::AreEqual(t3.getAttributes().size(),(size_t) 2); //Cross join with empty table should yield sum of attributes
		}
		...
		TEST_METHOD(crossJoin_FullEmptyA2){ 
			...
			s1.push_back(Table::TYPE::STRING);
			Table t1(m1, s1);
			Table t2 = t.crossJoin(t, t1);
			Assert::AreEqual(t2.getAttributes().size(),(size_t) 2); //Cross join with an empty table should yield sum of attributes 
		}
